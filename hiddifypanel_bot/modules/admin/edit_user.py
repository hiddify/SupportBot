...
def user_resp_from_inline_query(msg: HMessage):
    key = msg.reply_markup.keyboard[0][0]
    uuid = key.callback_data.split("_")[-1]
    bot.delete_message(msg.chat_id, msg.message_id)
    user = msg.hapi.get_user(uuid)
    send_user_info(msg, user)


def send_user_info(msg, user: User):
    resp = tghelper.format_user_message_from_admin(msg.lang, user)
    keyboards = types.InlineKeyboardMarkup()
    keyboards.add(
        types.InlineKeyboardButton(text=_("admin.open_admin"), web_app=types.WebAppInfo(f"{msg.db['admin_link']}/admin/user/edit/?id={user['id']}")),
    ...
    bot.send_message(msg.chat_id, resp, reply_markup=keyboards)


@bot.callback_query_handler(call_action=C.USER_ENABLE, role=Role.AGENT)
def enable_user_handler(call: HCallbackQuery):
    uuid = call.data.split(":")[-1]
    user = call.message.hapi.update_user(uuid, {"enable": True})
    send_user_info(call.message, user)
