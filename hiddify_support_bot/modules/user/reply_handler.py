...
async def is_reply_to_us_condition(msg:HMessage):
    if not msg or not msg.chat or not msg.from_user:return False
...
async def is_reply_to_user_condition(msg:HMessage):
    ...
    if msg.from_user.id == msg.chat.id:
        return False
    if await msg.db.get(f"chat_data_of_+{msg.reply_to_message.id}"):return True
    try:    
        url=urlparse(msg.reply_to_message.entities[0].url)
        query_params = parse_qs(url.query)
